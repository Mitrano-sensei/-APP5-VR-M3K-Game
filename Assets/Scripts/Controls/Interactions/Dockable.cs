...
public class Dockable : Pickable
{
    ...
    [SerializeField] private Vector3 _centerPosition = new Vector3(0, 0, 0);
    ...
    [Header("Constraints")]
    [SerializeField] private List<Vector2> _constraints = new List<Vector2>();
    ...
    private void ConstrainedDockToObject(Docker docker)
    {
        ...
        if (!dockManager.IsDockable(new Vector3(docker.X, docker.Y, docker.Z), _constraints))
        {
